...
def _normalize_text(text):
    text = ''.join(filter(lambda x: x in (
        string.digits + string.ascii_letters), text))
    return text.lower()
...
def _lexicon_search(lexicon, word):
    ...
    for lex_word in lexicon:
        edit_distances.append(editdistance.eval(
            _normalize_text(lex_word), _normalize_text(word)))
...
def Accuracy_with_lexicon(output, target, dataset=None, file_names=None):
    ...
    if len(file_names) == 0 or len(dataset.lexicons50[file_names[0]]) == 0:
        ...
        refined_pred_list = [_lexicon_search(
            dataset.lexicons50[file_name], pred) for file_name, pred in zip(file_names, pred_list)]
        acc_list = [(pred == targ)
    ...
    if len(file_names) == 0 or len(dataset.lexicons1k[file_names[0]]) == 0:
        ...
        refined_pred_list = [_lexicon_search(
            dataset.lexicons1k[file_name], pred) for file_name, pred in zip(file_names, pred_list)]
        acc_list = [(pred == targ)
                    for pred, targ in zip(refined_pred_list, targ_list)]
        accuracy = 1.0 * sum(acc_list) / len(acc_list)
        accuracys.append(accuracy)
    ...
    if len(file_names) == 0 or len(dataset.lexiconsfull[file_names[0]]) == 0:
        ...
        refined_pred_list = [_lexicon_search(
            dataset.lexiconsfull[file_name], pred) for file_name, pred in zip(file_names, pred_list)]
        acc_list = [(pred == targ)
                    for pred, targ in zip(refined_pred_list, targ_list)]
        accuracy = 1.0 * sum(acc_list) / len(acc_list)
        accuracys.append(accuracy)
...
def EditDistance(output, target, dataset=None):
    ...
    ed_list = [editdistance.eval(pred, targ)
