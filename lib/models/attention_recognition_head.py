...
class AttentionRecognitionHead(nn.Module):
    ...
    def beam_search(self, x, beam_width, eos, embed):
        ...
        pos_index = (torch.Tensor(range(batch_size))
                     * beam_width).long().view(-1, 1)

        # Initialize the scores
        sequence_scores = torch.Tensor(batch_size * beam_width, 1)
        sequence_scores.fill_(-float('Inf'))
        sequence_scores.index_fill_(0, torch.Tensor(
            [i * beam_width for i in range(0, batch_size)]).long(), 0.0)
        ...
        sorted_score, sorted_idx = stored_scores[-1].view(
        ...
        t_predecessors = (
            sorted_idx + pos_index.expand_as(sorted_idx)).view(batch_size * beam_width)
        while t >= 0:
            # Re-order the variables with the back pointer
            current_symbol = stored_emitted_symbols[t].index_select(
                0, t_predecessors)
            ...
            if eos_indices.dim() > 0:
                for i in range(eos_indices.size(0)-1, -1, -1):
                    ...
                    res_k_idx = beam_width - \
                        (batch_eos_found[b_idx] % beam_width) - 1
        ...
        re_sorted_idx = (
            re_sorted_idx + pos_index.expand_as(re_sorted_idx)).view(batch_size*beam_width)

        # Reverse the sequences and re-order at the same time
        # It is reversed because the backtracking happens in reverse time order
        p = [step.index_select(0, re_sorted_idx).view(
            batch_size, beam_width, -1) for step in reversed(p)]
        p = torch.cat(p, -1)[:, 0, :]
...
class DecoderUnit(nn.Module):
    def __init__(self, sDim, xDim, yDim, attDim):
        ...
        self.tgt_embedding = nn.Embedding(
            yDim+1, self.emdDim)  # the last is used for <BOS>
        self.gru = nn.GRU(input_size=xDim+self.emdDim,
                          hidden_size=sDim, batch_first=True)
    ...
    def get_initial_state(self, embed, tile_times=1):
        ...
        if tile_times != 1:
            state = state.unsqueeze(1).permute(1, 0, 2).repeat(
                tile_times, 1, 1).permute(1, 0, 2).contiguous().view(-1, self.sDim)
    ...
    def forward(self, x, sPrev, yPrev):
        ...
        output, state = self.gru(
            torch.cat([yProj, context], 1).unsqueeze(1), sPrev)
