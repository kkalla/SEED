...
class BaseEvaluator(object):
    ...
    def evaluate(self, data_loader, step=1, print_freq=1,
                 tfLogger=None, dataset=None, vis_dir=None):
        self.model.eval()
        ...
        if 'pred_rec' in outputs:
            ...
            if global_args.evaluate_with_lexicon:
                ...
                print('lexicon0: {0}, {1:.3f}'.format(
                    self.metric, eval_res[0]))
                print('lexicon50: {0}, {1:.3f}'.format(
                    self.metric, eval_res[1]))
                print('lexicon1k: {0}, {1:.3f}'.format(
                    self.metric, eval_res[2]))
                print('lexiconfull: {0}, {1:.3f}'.format(
        ...
        if 'pred_embed' in outputs:
            output_cos_sim = self.cos_sim(
                outputs['pred_embed'], torch.cat(embeds).cpu())
            output_cos_sim = torch.mean(torch.abs(output_cos_sim))
            print("Emebedding vector cos similarity: {:3f}".format(
                output_cos_sim.item()))
