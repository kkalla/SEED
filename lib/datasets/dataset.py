...
class CustomDataset(data.Dataset):
    def __init__(self, root, gt_file_path, embed_path, voc_type, max_len, num_samples, transform=None):
        ...
        if os.path.basename(gt_file_path).split(".")[-1] == "json":
            self.images_path, self.transcriptions, self.embeds_path = self.load_gt_json(
                gt_file_path)
        elif os.path.basename(gt_file_path).split(".")[-1] == "txt":
            self.images_path, self.transcriptions, self.embeds_path = self.load_gt_txt(
                gt_file_path)
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if embed_path is not None:
                embed_vector = np.load(
                    os.path.join(self.embed_path, embed_path))
            else:
      ...
      """
            ...
            if self.embed_path is None:
                ...
                if not os.path.exists(os.path.join(self.embed_path, k.replace("jpg", "npy"))):
                    embed_file_path = k.split(
                        "/")[5] + "/" + k.split("/")[6].replace("jpg", "npy")
...
def debug():
    ...
    train_dataset = CustomDataset(root=img_root_dir, gt_file_path=gt_file_path,
                                  voc_type="ALLCASES_SYMBOLS", max_len=50, num_samples=5000)
    batch_size = 4
